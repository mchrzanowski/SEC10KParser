...
def _default_case_sensitive():
    ...
    '''
    return re.compile("^\s*ITEM\s*?3[^A][.\s]*?[^,].*?(?=^\s*?ITEM\s*?(3A|4))", re.M | re.S), None

def _item_4_is_skipped_case_sensitive():
    return re.compile("^\s*ITEM\s*?3[^A][.\s]*?[^,].*?(?=^\s*?ITEM\s*?[5-6])", re.I | re.M | re.S), re.compile("ITEM\s*4", re.I | re.M)

def _item_4_is_skipped_case_insensitive():
    return re.compile("^\s*ITEM\s*?3[^A][.\s]*?[^,].*?(?=^\s*?ITEM\s*?[5-6])", re.M | re.S), re.compile("ITEM\s*4", re.I | re.M)

def _default_case_insensitive():
    return re.compile("^\s*ITEM\s*?3[^A][.\s]*?[^,].*?(?=^\s*ITEM\s*?(3A|4))", re.I | re.M | re.S), None 
...
def _try_all_numbers_after_4_and_executive_listing():
    ...
    '''
    return re.compile(  \
        "^\s*[iI][tT][eE][mM]\s*?3[^A][.\s]*?[^,].*?(?=([iI][tT][eE][mM]\s*?[5-6]|E[xX][eE][cC][uU][tT][iI][vV][eE]\s*?O[fF][fF][iI][cC][eE][rR][sS]))", \
...
def _item_3_is_last_item():
    ...
    '''
    return re.compile("^\s*[iI][tT][eE][mM]\s*?3[^A][.\s]*?[^,].*?(?=^\s*?E[xX][eE][cC][uU][tT][iI][vV][eE]\s*?O[fF][fF][iI][cC][eE][rR][sS])", re.M | re.S), \
        re.compile("ITEM\s*4", re.I | re.M)

def _item_3_is_lodged_somewhere_case_sensitive_and_is_last_item_case_sensitive():
    return re.compile("ITEM\s*?3[^A][.\s]*?[^,].*?(?=^\s*?E[xX][eE][cC][uU][tT][iI][vV][eE]\s*?O[fF][fF][iI][cC][eE][rR][sS])", re.M | re.S), \
        re.compile("ITEM\s*4", re.I | re.M)

def _item_3_is_lodged_somewhere_case_sensitive_and_item_4_is_skipped():
    return re.compile("ITEM\s*?3[^A][.\s]*?[^,].*?(?=^\s*?ITEM\s*?[5-6])", re.M | re.S), re.compile("ITEM\s*4", re.I | re.M)
...
def _item_3_is_lodged_somewhere_case_sensitive():
    ...
    '''
    return re.compile("ITEM\s*?3[^A][.\s]*?[^,].*?(?=^\s*ITEM\s*?(3A|4))", re.M | re.S), None

def _item_3_is_lodged_somewhere_case_sensitive_no_newlines():
    return re.compile("ITEM\s*?3[^A][.\s]*?[^,].*?(?=ITEM\s*?(3A|4))", re.M | re.S), None

def _item_3_is_lodged_somewhere_and_is_last_item_case_sensitive_no_newlines():
    return re.compile("ITEM\s*?3[^A][.\s]*?[^,].*?(?=E[xX][eE][cC][uU][tT][iI][vV][eE]\s*?O[fF][fF][iI][cC][eE][rR][sS])", re.M | re.S), \
        re.compile("ITEM\s*4", re.I | re.M)

def _item_3_is_lodged_somewhere_and_item_4_is_skipped_case_sensitive_no_newlines():
    return re.compile("ITEM\s*?3[^A][.\s]*?[^,].*?(?=ITEM\s*?[5-6])", re.M | re.S), re.compile("ITEM\s*4", re.I | re.M)
...
def _item_3_is_lodged_somewhere_case_insensitive_with_punctuation():
    ...
    return re.compile("(?<=[.?!])\s\s+?ITEM\s*?3[^A][.\s]*?[^,].*?(?=ITEM\s*?(3A|4))", re.I | re.M | re.S), None

def _item_3_is_lodged_somewhere_and_item_4_is_skipped_case_insensitive_with_punctuation():
    return re.compile("(?<=[.?!])\s\s+?ITEM\s*?3[^A][.\s]*?[^,].*?(?=ITEM\s*?[5-6])", re.I | re.M | re.S), re.compile("ITEM\s*4", re.I | re.M)

def _item_3_is_lodged_somewhere_and_is_last_case_insensitive_with_punctuation():
    return re.compile("(?<=[.?!])\s\s+?[iI][tT][eE][mM]\s*?3[^A][.\s]*?[^,].*?(?=E[xX][eE][cC][uU][tT][iI][vV][eE]\s*?O[fF][fF][iI][cC][eE][rR][sS])", \
                      re.M | re.S), re.compile("ITEM\s*4", re.I | re.M)
...
def _item_3_is_lodged_somewhere_case_insensitive_with_punctuation_no_spaces():
    ...
    '''
    return re.compile("(?<=[.?!])\s*?ITEM\s*?3[^A][.\s]*?[^,].*?(?=ITEM\s*?(3A|4))", re.I | re.M | re.S), None

def _item_3_is_lodged_somewhere_and_item_4_is_skipped_case_insensitive_with_punctuation_no_spaces():
    return re.compile("(?<=[.?!])\s*?ITEM\s*?3[^A][.\s]*?[^,].*?(?=ITEM\s*?[5-6])", re.I | re.M | re.S), re.compile("ITEM\s*4", re.I | re.M)

def _item_3_is_lodged_somewhere_and_is_last_case_insensitive_with_punctuation_no_spaces():
    return re.compile("(?<=[.?!])\s*?[iI][tT][eE][mM]\s*?3[^A][.\s]*?[^,].*?(?=E[xX][eE][cC][uU][tT][iI][vV][eE]\s*?O[fF][fF][iI][cC][eE][rR][sS])", re.M | re.S), \
        re.compile("ITEM\s*4", re.I | re.M)

def _item_3_is_lodged_somewhere_assume_legal_proceeding_is_capitalized():
    return re.compile("[iI][tT][eE][mM]\s*?3[^A][\s.]*?LEGAL\s*?PROCEEDING.*?(?=[iI][tT][eE][mM]\s*?4)", re.M | re.S), None

def _item_3_is_lodged_somewhere_assume_item_is_capitalized():
    return re.compile("ITEM\s*?3[^A][\s.]*?[lL][eE][gG][aA][lL]\s*?[pP][rR][oO][cC][eE][eE][dD][iI][nN][gG].*?(?=ITEM\s*?4)", re.M | re.S), None
